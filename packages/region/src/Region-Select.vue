...
import ElSelectMenu from '../../select/src/select-dropdown.vue';
...
import NavigationMixin from '../../select/src/navigation-mixin';
import { isKorean } from 'element-ui/src/utils/shared';

export default {
  mixins: [Emitter, Locale, Focus('reference'), NavigationMixin],
  name: 'ElRegion',
  componentName: 'ElRegion',
  inject: {
    elForm: {
      default: ''
    },
  ...
  data() {
    let first = this.regionData;
    return {
      /* region */
      currentModelLabel: '' /* 用来显示地址的字符串 */,
      activeName: 'first',
  ...
  watch: {
    ...
    valueLabel(value) {
      if (Array.isArray(value) && value.length > 0) {
        this.currentModelLabel = value.join('-');
      }
    },
  ...
  methods: {
    ...
    scrollToOption(option) {
      ...
      this.$refs.scrollbar &&
        this.$refs.scrollbar.forEach(sb => {
          sb.handleScroll();
        });
    },
    ...
    handleBlur(event) {
      console.log('%chandleBlur', 'colr: green');
      setTimeout(() => {
